...
- **AI Analysis section** (industry reports, OverviewTab): GPT generates summary with sections: Market leader, Competitive landscape, Provider differences, Sentiment patterns, Source patterns, Key insight. Text corrections in `metrics/compute/textCorrections.ts`:
  - **Execution order in `correctIndustryAISummary`**: (1) `correctBrandMetricsInText` fixes GPT's raw numbers FIRST, (2) ML injection appends deterministic visibility score + share of voice to Market leader paragraph, (3) Competitive landscape paragraph is rebuilt with deterministic text, (4) brand annotations added, (5) visibility score definition added if not already present.
  - **`findNearestBrand`**: Uses word-boundary regex (`\b`) to find brands in text. For short brands (≤3 chars like "On"), also requires capitalization to avoid matching common words inside other words (e.g., "On" inside "recommendations").
  - **`isNearProvider` guard**: Steps 3c, 4, 5 skip percentage replacement when a provider name (ChatGPT, Claude, Gemini, Perplexity, Grok, Llama, AI Overviews) appears within 120 chars — per-provider stats from backend data are correct and should NOT be replaced with overall visibility.
  - **Visibility score vs share of voice**: Two distinct metrics. Visibility = % of AI responses that mention the brand. Share of voice = brand's mentions / total brand mentions (shares sum to ~100%). ML injection labels both clearly with inline definitions. Backend sends both metrics separately to GPT with explicit instructions not to confuse them.
  - **Share of voice correction** (steps 3a/3b): Catches "XX% share of voice" patterns and replaces with correct `shareOfVoice` from stats — separate from visibility correction.
  - Backend "mention rate" text is replaced with "visibility score". Repeated "suggest/suggests" is varied with alternatives (indicates, points to, reflects). Does NOT list all brands — full brand breakdown is in the Competitive tab.
