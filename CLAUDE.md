...
- **AI Analysis section** (industry reports, OverviewTab): GPT generates qualitative summary (no numbers) with 4 sections: Market leader, Provider differences, Sentiment patterns, Key insight. Text corrections in `metrics/compute/textCorrections.ts`:
  - **`correctIndustryAISummary`**: (1) Prepends a deterministic stats header with top 5 brands + visibility scores from `BrandBreakdownRow[]`, (2) strips stray percentages GPT may have included despite prompt instructions, (3) normalises "mention rate" → "visibility score".
  - **`correctBrandMetricsInText`** (shared): Used by RecommendationsTab (Analysis tab) to fix GPT-hallucinated percentages. Has `findNearestBrand`, `isNearProvider` guard, share-of-voice corrections, brand count fixes, and visibility/mention-rate metric replacement. NOT used by the AI Analysis section anymore.
  - **Visibility score vs share of voice**: Two distinct metrics. Visibility = % of AI responses that mention the brand. Share of voice = brand's mentions / total brand mentions (shares sum to ~100%).
