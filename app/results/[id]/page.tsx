...
export default function ResultsPage() {
  ...
  const SentimentTab = () => {
    ...
    // Calculate sentiment by prompt
    const sentimentByPrompt = useMemo(() => {
      ...
      globallyFilteredResults
        ...
        .forEach((r: Result) => {
          ...
          const sentiment = r.brand_sentiment || 'not_mentioned';
          if (sentiment in promptData[prompt]) {
            promptData[prompt][sentiment as keyof Omit<typeof promptData[string], 'prompt'>]++;
          }
        });

      return Object.values(promptData)
        .map((data) => {
          const total = data.strong_endorsement + data.positive_endorsement +
                       data.neutral_mention + data.conditional +
                       data.negative_comparison + data.not_mentioned;
          const positiveTotal = data.strong_endorsement + data.positive_endorsement;
          return {
            ...data,
            total,
            positiveRate: total > 0 ? (positiveTotal / total) * 100 : 0,
            truncatedPrompt: data.prompt.length > 40 ? data.prompt.substring(0, 40) + '...' : data.prompt,
          };
        })
    ...
    return (
      <div className="space-y-6">
        ...
        {/* Sentiment by Prompt */}
        {sentimentByPrompt.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-1">Sentiment by Prompt</h3>
            <p className="text-sm text-gray-500 mb-6">How different prompts generate favorable or unfavorable responses</p>

            <div className="space-y-4">
              {sentimentByPrompt.map((data, idx) => {
                  ...
                  <div key={idx} className="group">
                    <div className="flex items-center gap-4 mb-2">
                      <div className="w-64 shrink-0">
                        <p className="text-sm text-gray-700 truncate" title={data.prompt}>
                          {data.truncatedPrompt}
                        </p>
                      </div>
                      <div className="flex-1 flex items-center gap-1 h-6">
                        {data.strong_endorsement > 0 && (
                          <div
                            className="h-full bg-green-500 rounded-l first:rounded-l last:rounded-r transition-all"
                            style={{ width: `${(data.strong_endorsement / total) * 100}%` }}
                            title={`Very Favorable: ${data.strong_endorsement}`}
                          />
                        )}
                        {data.positive_endorsement > 0 && (
                          <div
                            className="h-full bg-lime-500 first:rounded-l last:rounded-r transition-all"
                            style={{ width: `${(data.positive_endorsement / total) * 100}%` }}
                            title={`Favorable: ${data.positive_endorsement}`}
                          />
                        )}
                        {data.neutral_mention > 0 && (
                          <div
                            className="h-full bg-blue-500 first:rounded-l last:rounded-r transition-all"
                            style={{ width: `${(data.neutral_mention / total) * 100}%` }}
                            title={`Neutral: ${data.neutral_mention}`}
                          />
                        )}
                        {data.conditional > 0 && (
                          <div
                            className="h-full bg-yellow-500 first:rounded-l last:rounded-r transition-all"
                            style={{ width: `${(data.conditional / total) * 100}%` }}
                            title={`Conditional: ${data.conditional}`}
                          />
                        )}
                        {data.negative_comparison > 0 && (
                          <div
                            className="h-full bg-red-500 first:rounded-l last:rounded-r transition-all"
                            style={{ width: `${(data.negative_comparison / total) * 100}%` }}
                            title={`Negative: ${data.negative_comparison}`}
                          />
                        )}
                        {data.not_mentioned > 0 && (
                          <div
                            className="h-full bg-gray-300 rounded-r first:rounded-l last:rounded-r transition-all"
                            style={{ width: `${(data.not_mentioned / total) * 100}%` }}
                            title={`Not Mentioned: ${data.not_mentioned}`}
                          />
                        )}
                      </div>
                      <div className="w-16 text-right shrink-0">
                        <span className={`text-sm font-medium ${
                          data.positiveRate >= 50 ? 'text-green-600' :
                          data.positiveRate >= 25 ? 'text-yellow-600' :
                          'text-gray-500'
                        }`}>
                          {data.positiveRate.toFixed(0)}%
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            {/* Legend */}
            <div className="flex flex-wrap items-center gap-4 mt-6 pt-4 border-t border-gray-100 text-xs">
