...
export const OverviewTab = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Issue Positioning Chart — Mention Count vs Framing */}
      {showSection('issue-positioning') && isIssue && (() => {
        const posData: { issue: string; mentions: number; sentiment: number; isMainIssue: boolean }[] = overviewMetrics?.issuePositioningData || [];
        if (posData.length === 0) return null;

        // Dynamic axis ranges
        const sentimentValues = posData.map(d => d.sentiment);
        ...
        const maxMentions = Math.max(...posData.map(d => d.mentions));
        const yMax = maxMentions + Math.max(1, Math.ceil(maxMentions * 0.15));

        // Cluster overlapping points
        const getClusterKey = (mentions: number, sentiment: number) => {
          const mBucket = Math.round(mentions);
          const sBucket = Math.round(sentiment * 2) / 2;
          return `${mBucket}-${sBucket}`;
        };
        const groups: Record<string, typeof posData> = {};
        posData.forEach(point => {
        ...
        const processedData = posData.map(point => {
          const key = getClusterKey(point.mentions, point.sentiment);
          const group = groups[key];
          const indexInGroup = group.findIndex(p => p.issue === point.issue);
          return { ...point, groupSize: group.length, indexInGroup };
        });

        return (
          <div id="overview-issue-positioning" className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Issue Landscape</h2>
                <p className="text-sm text-gray-500 mt-1">
                  How often AI discusses each issue vs. how it frames them. Top-right = most discussed with supportive framing.
                </p>
              </div>
            </div>
            {/* Legend */}
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-4">
              <span className="text-xs text-gray-600 font-medium">Framing:</span>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 30, right: 40, bottom: 60, left: 60 }}>
                  ...
                  <XAxis
                    ...
                    domain={[xMin, xMax]}
                    ticks={xTicks}
                    tickFormatter={(value) => ['', 'Critical', 'Mixed', 'Balanced', 'Leaning Supportive', 'Supportive'][value] || ''}
                    tick={{ fill: '#6b7280', fontSize: 12 }}
                    label={{ value: 'Average Framing', position: 'bottom', offset: 25, style: { fill: '#374151', fontSize: 14, fontWeight: 500 } }}
                  />
                  <YAxis
                    type="number"
                    dataKey="mentions"
                  ...
                  <Tooltip
                    ...
                    content={({ active, payload }) => {
                      if (active && payload && payload.length > 0) {
                        ...
                        const framingLabels = ['', 'Critical', 'Mixed', 'Balanced', 'Leaning Supportive', 'Supportive'];
                        return (
                          <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-sm max-w-[300px]">
                            <p className="font-medium text-gray-900 mb-2">{data.issue}{data.isMainIssue ? ' (main issue)' : ''}</p>
                            <div className="space-y-1 text-gray-600">
                              <p>Framing: <span className="font-medium">{framingLabels[Math.round(data.sentiment)] || 'N/A'}</span></p>
                  ...
                  <Scatter
                    ...
                    shape={(props: any) => {
                      ...
                      const isMain = payload.isMainIssue;
                      ...
                      const circleRadius = isMain ? 11 : 9;
                      const spacing = 22;
                      const totalWidth = (groupSize - 1) * spacing;
                      const xOffset = groupSize > 1 ? (indexInGroup * spacing) - (totalWidth / 2) : 0;

                      const shortIssue = payload.issue.length > 18
                        ? payload.issue.substring(0, 16) + '...'
                        : payload.issue;
                      ...
                      return (
                        <g>
                          <circle
                            ...
                            r={circleRadius}
                            fill={getColor()}
                            stroke={isMain ? '#1f2937' : getStroke()}
            ...
            <p className="text-xs text-gray-400 italic text-center mt-2">Hover over dots for details {'\u2022'} Main issue has thicker border</p>
          </div>
        );
      })()}
